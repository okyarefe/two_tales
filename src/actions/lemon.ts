...
export async function syncPlans() {
  ...
  const pricePromises = variantsToProcess.map((item) =>
    listPrices({ filter: { variantId: item.variantId } })
      ...
      .catch((err) => {
        console.error(
          `❌ Error fetching price for variant ${item.variantId}:`,
          err,
        );
  ...
  for (const item of variantsWithPrices) {
    ...
    const currentPriceObj =
      prices.find(
        (p) =>
          p.attributes?.currency === 'EUR' && p.attributes?.status === 'active',
      ) ||
...
export async function getCheckoutURL(variantId: number, embed = false) {
  // console.log('+++++++++++++++++Get Checkout URL RAN!');
  // console.log('++++++++  V A R I A N T ID ', variantId);
  ...
  // console.log('--Checkout--', checkout.error);

  if (checkout.error) {
    // console.error('❌ Lemon Squeezy createCheckout error:', {
    //   message: checkout.error.message,
    //   statusCode: checkout.statusCode,
    //   storeId: process.env.LEMONSQUEEZY_STORE_ID,
    //   variantId,
    //   cause: JSON.stringify(
    //     (checkout.error as Error & { cause?: unknown }).cause,
    //     null,
    //     2,
    //   ),
    //   data: JSON.stringify(checkout.data, null, 2),
    // });
  ...
  if (!url) {
    console.error('❌ Lemon Squeezy returned no checkout URL:', checkout.data);
    throw new Error('Checkout URL not returned by Lemon Squeezy');
  }
