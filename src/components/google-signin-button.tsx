...
const variantStyles: Record<Variant, string> = {
  signin: 'bg-accent text-primary-foreground hover:bg-accent/90',
...
export default function GoogleSignInButton({
  ...
  const baseSizing =
    variant === 'learn'
      ...
      : 'inline-flex items-center gap-2 rounded-md text-xs md:text-sm h-8 px-2 sm:px-3 md:px-6';

  return (
    <Button
      size={variant === 'learn' ? 'lg' : 'sm'}
      className={`${variantStyles[variant]} ${baseSizing} ${className}`.trim()}
